 ...
 */
public class CreateExtOrderCommand extends FleetHystrixCommand<OrderApiResponse> {

  private final FleetExtDeliveryRequest request;

  private final RestTemplate restTemplate;

  public CreateExtOrderCommand(FleetExtDeliveryRequest request, RestTemplate restTemplate,
                               TransporterApiMetadata apiMetadata) {
    super(HystrixCommandGroupKey.Factory.asKey(FleetConstants.CLIENT_ID), 25000, apiMetadata);
    this.request = request;
    this.restTemplate = restTemplate;
  }

  @Override
  protected OrderApiResponse run() throws Exception {

    HttpEntity<FleetExtDeliveryRequest> httpRequest = new HttpEntity<>(request, headers());
    try {
      ResponseEntity<OrderApiResponse> entity =
          restTemplate.postForEntity(url(), httpRequest, OrderApiResponse.class);
  ...
  private String url() {
    return getBaseUrl() + FleetConstants.ORDER_ENDPOINT;
  }
