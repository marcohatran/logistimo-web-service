 ...
 */
public class MobileModelMapperUtils {

  public static ConsignmentModel buildConsignmentModelFromMobileModel(
      MobileConsignmentModel consignment) {
    return ConsignmentModel.builder()
        .id(consignment.getId())
        .declaration(consignment.getContentDeclaration())
        .dimension(new PackageDimensions(
            consignment.getLength(), consignment.getWidth(), consignment.getHeight()))
        .packageCount(consignment.getPackageCount())
        .value(consignment.getValue())
        .weightInKg(consignment.getWeightInKg())
        .build();
  }
}